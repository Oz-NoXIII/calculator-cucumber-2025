...
class TestSinus(unittest.TestCase):
    ...
    def setUp(self):
        ...
        params4 = [self.value4]
        try:
            self.op = Sinus(params)
            self.op2 = Sinus(params2)
            self.op3 = Sinus(params3)
            self.op4 = Sinus(params4)
        except IllegalConstruction as e:
            self.fail(e)

    def test_constructor(self):
        self.assertRaises(IllegalConstruction, lambda: Sinus(None))
        try:
    ...
    def test_sinus(self):
        ...
        result = Sinus([p], Notation.INFIX)
        self.assertEqual(RealNumber(0).sin().get_value(), result.op(RealNumber(0)).get_value())

        p2 = MyNumber(IntegerNumber(0))
        result2 = Sinus([p2], Notation.INFIX)
        self.assertEqual(IntegerNumber(0).sin().get_value(), result2.op(IntegerNumber(0)).get_value())

        p = MyNumber(ComplexNumber(0, 0))
        result = Sinus([p], Notation.INFIX)
        self.assertEqual(ComplexNumber(0, 0).sin().get_value(), result.op(ComplexNumber(0, 0)).get_value())

        p = MyNumber(RationalNumber(0, 1))
        result = Sinus([p], Notation.INFIX)
        self.assertEqual(RationalNumber(0, 1).sin().get_value(), result.op(RationalNumber(0, 1)).get_value())

    def test_equals(self):
        p = [self.value1]
        try:
            e = Sinus(p, Notation.INFIX)
