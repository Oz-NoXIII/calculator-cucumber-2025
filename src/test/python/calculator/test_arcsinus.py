...
class TestSinus(unittest.TestCase):
    ...
    def setUp(self):
        ...
        params4 = [self.value4]
        try:
            self.op = Arcsinus(params)
            self.op2 = Arcsinus(params2)
            self.op3 = Arcsinus(params3)
            self.op4 = Arcsinus(params4)
        except IllegalConstruction as e:
            self.fail(e)

    def test_arcsinus(self):
        p = MyNumber(RealNumber(0))
        result = Arcsinus([p], Notation.INFIX)
        self.assertEqual(RealNumber(0).arcsin().get_value(), result.op(RealNumber(0)).get_value())

        p2 = MyNumber(IntegerNumber(0))
        result2 = Arcsinus([p2], Notation.INFIX)
        self.assertEqual(IntegerNumber(0).arcsin().get_value(), result2.op(IntegerNumber(0)).get_value())

        p = MyNumber(ComplexNumber(0, 0))
        result = Arcsinus([p], Notation.INFIX)
        self.assertEqual(ComplexNumber(0, 0).arcsin().get_value(), result.op(ComplexNumber(0, 0)).get_value())

        p = MyNumber(RationalNumber(0, 1))
        result = Arcsinus([p], Notation.INFIX)
        self.assertEqual(RationalNumber(0, 1).arcsin().get_value(), result.op(RationalNumber(0, 1)).get_value())

    def test_constructor(self):
        self.assertRaises(IllegalConstruction, lambda: Arcsinus(None))
        try:
    ...
    def test_none_param_list(self):
        params = None
        self.assertRaises(IllegalConstruction, lambda: Arcsinus(params))
